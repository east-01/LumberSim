...
///   prefab.
/// </summary>
public class TreeLogSpawner : MonoBehaviour, IS3
{
    /// <summary>
    ...
    private TreeLogGroup logGroup;

    [SerializeField]
    private TreeLogData rootData;
    ...
    void Update()
    {
        ...
        if(gameplayManager != null && logGroup == null && (Time.time - startTime) > 0.3f) {
            logGroup = gameplayManager.SpawnLogObject(transform.position, Quaternion.identity, rootData);
        }
    }
}
